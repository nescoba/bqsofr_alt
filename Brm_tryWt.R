...
for(l in 1:length(tauVal)){ 
  ...
  */
    ...
    StandCodeVGalLin <- "
      ...
      generated quantities {
        ...
        vector[N] log_lik;
          mu += Intercept + Xc * b + Wn * bwa;
        for(n in 1:N)
              log_lik[n] +=  GAL2_lpdf(Y[n] | mu[n], sigma[l], gam[l], tau0);
  ...
  form1 <- paste0("Y ~ ", paste(c("Gender","Race","HealthCondt2","AgeYR", colnames(df2)[grepl("W",colnames(df2))]), collapse = " + "))
  Dt2 <- make_standata(bf(as.formula(form1), quantile = tau0), data = df2[c(1:nrow(df2)),], family = asym_laplace())
