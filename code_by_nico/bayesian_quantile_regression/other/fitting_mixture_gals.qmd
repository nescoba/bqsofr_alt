...
## Fiting mixture of GALS

Creating the mixture object:

```{r}
mix <- mixture(GAL2, GAL2)
```

Generating synthetic data. This will be a two way mixture. 
...
samples <- data.frame(y =rnorm(n=N,mean=mus[components],sd=.2), x = 1)
```

Fitting the mixture 

```{r}
prior <- c(prior(normal(0,1),Intercept,dpar = mu1), prior(normal(0,1), Intercept, dpar = mu2))
fit <- brm(bf(y~1, mu1~1, mu2~1), cores = 2, data = samples, family = mix, prior = prior,stanvars = stanvars2, chains = 2, iter = 1000)
summary(fit)
```
