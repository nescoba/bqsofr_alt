...
I had the thought that maybe we're asking too much of the model. It has to make inferences about the coefficients for $y$, for $\sigma$ and figure out the $\gamma$. We could make the model's task simpler by considering the .5 quartile, whose location does not depend of $x$. 

Here's the original model, for reference: 
...
q50 <- brm(bf(y ~ x, sigma ~ log(x) - 1, tau = .5), data = synthetic, family = GAL2, stanvars = stanvars2, chains = 2, iter = 500, control = list(adapt_delta = 0.99), cores = 4, seed = 123)
summary(q50)
```
...
None of it is good, but the $\gamma$ is particularly bad. 
...
So, let's tell the model that the location is constant:
...
Still a lot of divergences. 

```{r}
plot(q50)
```

The intercept is ok, but the other two have problems 
...
q50 <- brm(bf(y ~ 1, sigma ~ x - 1, tau = .5), data = synthetic, family = GAL2, stanvars = stanvars2, chains = 2, iter = 500, control = list(adapt_delta = 0.99), cores = 4, seed = 123)
summary(q50)
```
...
q50 <- brm(bf(y ~ 1, sigma ~ x - 1, tau = .5), data = synthetic, family = GAL2, stanvars = stanvars2, chains = 2, iter = 1000, control = list(adapt_delta = 0.99), cores = 4, seed = 123)
summary(q50)
```
